...
CREATE OR REPLACE FUNCTION fuelos_station_id() RETURNS UUID
  LANGUAGE sql STABLE SECURITY DEFINER AS $$
    SELECT (auth.jwt() -> 'user_metadata' ->> 'station_id')::UUID
  $$;
...
CREATE OR REPLACE FUNCTION fuelos_user_id() RETURNS UUID
  LANGUAGE sql STABLE SECURITY DEFINER AS $$
    SELECT (auth.jwt() -> 'user_metadata' ->> 'fuelos_user_id')::UUID
  $$;
...
GRANT EXECUTE ON FUNCTION fuelos_creditor_lookup(UUID, TEXT) TO anon;
...
GRANT EXECUTE ON FUNCTION fuelos_creditor_transactions(UUID, UUID) TO anon;
...
GRANT EXECUTE ON FUNCTION fuelos_creditor_payments(UUID, UUID) TO anon;
