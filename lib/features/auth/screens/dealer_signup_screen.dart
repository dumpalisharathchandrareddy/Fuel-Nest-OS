...
class _DealerSignupScreenState extends ConsumerState<DealerSignupScreen> {
  ...
  void dispose() {
    for (final c in [
      _urlCtrl, _anonKeyCtrl, _codeCtrl, _nameCtrl,
      _cityCtrl, _stateCtrl, _ownerNameCtrl, _phoneCtrl,
  ...
  List<Widget> _buildStep0() => [
    ...
    AppTextField(
      ...
      validator: (v) {
        ...
        if (!v.trim().contains('supabase.co')) return 'Enter a valid Supabase URL';
        return null;
      },
    ),
    const SizedBox(height: 16),
