    ...
    try {
      ...
      final results = await Future.wait([
        db
            .from('Pump')
            .select(
                'id, name, provider_type, active, nozzles:Nozzle(id, label, fuel_type, active)')
            .eq('station_id', user.stationId),
        db
            .from('Tank')
            .select('id, name, fuel_type, capacity_liters, active')
            .eq('station_id', user.stationId),
      ]);
      setState(() {
        _pumps = results[0];
          ...
          ..._pumps.map((p) => Padding(
            ...
            child: AppCard(
                child: Column(
                  ...
                  ...(p['nozzles'] as List? ?? []).map((n) => Padding(
                      ...
                      child: Row(children: [
                        ...
                        const SizedBox(width: 8),
                        Expanded(
                            child: Text('${n['label']} — ${n['fuel_type']}',
                                style: const TextStyle(
                                    color: AppColors.textSecondary,
                                    fontSize: 13))),
                        StatusBadge(
                            label: n['active'] == true ? 'On' : 'Off',
                            tone: n['active'] == true
                                ? BadgeTone.success
                                : BadgeTone.error),
                      ]))),
